...
const AdminActors = () => {
    ...
    // 2. ЛОКАЛЬНА ПАГІНАЦІЯ (Фільми актора)
    const {
        items: actorMovies,
    ...
    } = useLocalPagination<ActorMovieDto>(
        async (page, size) => {
    ...
    const columns: Column<ActorDto>[] = [
        {
            ...
            render: (actor) => (
                <div className="w-12 h-12 rounded-2xl bg-gray-800 border border-gray-700 overflow-hidden flex items-center justify-center">
                    {actor.photoUri ? (
                        <img src={actor.photoUri.startsWith('http') ? actor.photoUri : `${API_CONFIG.BASE_URL}${actor.photoUri}`} className="w-full h-full object-cover" />
                    ) : (
                        ...
                        <div className="p-8 border-b border-gray-800 flex items-center gap-6 bg-linear-to-r from-red-600/10 to-transparent">
                            <div className="w-24 h-24 rounded-2xl border-2 border-red-600 p-1 overflow-hidden bg-gray-900 shrink-0">
                                {selectedActor.photoUri ? (
                                    <img src={selectedActor.photoUri.startsWith('http') ? selectedActor.photoUri : `${API_CONFIG.BASE_URL}${selectedActor.photoUri}`} className="w-full h-full object-cover rounded-xl" />
                                ) : (
