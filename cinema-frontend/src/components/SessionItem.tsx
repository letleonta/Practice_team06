import { Calendar, Clock, MapPin, Languages, Trash2 } from 'lucide-react';
import type {SessionDto} from "../types/session.ts";

interface SessionItemProps {
    session: SessionDto;
    onDelete?: (id: number) => void; // Опціонально для клієнта
    onClick?: (id: number) => void;  // Для клієнта (handleBuyTicket)
    variant?: 'admin' | 'client';    // Вибір стилю
}
...
export const SessionItem = ({ session, onDelete, onClick, variant = 'admin' }: SessionItemProps) => {
    ...
    // ВАРІАНТ ДЛЯ АДМІНА (Горизонтальна картка)
    return (
